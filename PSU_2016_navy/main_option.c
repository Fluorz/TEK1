...
** Last update Thu Feb 16 18:13:55 2017 Leo Lecherbonnier
*/
		      ...
		      t_player *player_2, char **av, int pid_player2)
{
  ...
  if ((pid_player2 = open(av[2], O_RDONLY)) == -1)
    return (84);
  pid_player2 = my_getnbr(av[1]);
  g_player.pid_adverse = pid_player2;
  kill(pid_player2, SIGUSR1);
  my_printf("successfully connected\n\n");
  check_first_connection(s_sa);
  status = player2(s_sa, player_2, av);
  if (status != 84)
    {
      if (status == 2)
	my_putstr("Enemy won\n");
